...
#include "spivideoplaycontrol.h"
#include "resource.h"

// Common files moved from folder C:\Program Files\Microsoft SDKs\Windows\v7.1\Samples\multimedia\directshow\common
#include "smartptr.h"
#include "dshowutil.h"
...
//spi, begin
char global_filename[1024]={"jesus-bd_pages.avi"};
...
HRESULT InitVideoWindow(int nMultiplier, int nDivider)
{
	...
	*/
    SetWindowPos(ghApp, NULL, global_x, global_y, global_xwidth, global_yheight,
                 SWP_NOMOVE | SWP_NOOWNERZORDER);
	//spi, end
...
void OpenClip()
{
	...
	*/
	...
	wcscpy(g_szFileName, mywstring.c_str());
    hr = PlayMovieInWindow(g_szFileName);
...
void CALLBACK StartGlobalProcess(UINT uTimerID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
	...
	PostMessage(global_hwnd, WM_COMMAND, ID_FILE_OPENCLIP, 0);
	...
	while( ((nowstamp_ms-global_startstamp_ms)/1000.0f)<global_duration_sec || (mediaduration_s<0.0f) || ((global_duration_sec<0.0f)&&(mediaduration_s>0.0f)&&(((nowstamp_ms-global_startstamp_ms)/1000.0f)<mediaduration_s)) )
	{
		...
		//if(1)
		{
			HRESULT hr = pMP->get_Duration(&myREFTIME);
			if(hr==S_OK)
			{
				bcalled=true;
				mediaduration_s = myREFTIME;
			}
		}
	}
	...
	*/
	...
	if(global_titlebardisplay)
	{
		...
		ghApp = CreateWindow(CLASSNAME, APPLICATIONNAME,
							 ...
							 //CW_USEDEFAULT, CW_USEDEFAULT, DEFAULT_AUDIO_WIDTH, DEFAULT_AUDIO_HEIGHT,
	...
	else
	{
		// Create the main window.  The WS_CLIPCHILDREN style is required.
		ghApp = CreateWindow(CLASSNAME, APPLICATIONNAME,
							 WS_POPUP, //WS_OVERLAPPEDWINDOW | WS_CAPTION | WS_CLIPCHILDREN | WS_VISIBLE,
							 //CW_USEDEFAULT, CW_USEDEFAULT, DEFAULT_AUDIO_WIDTH, DEFAULT_AUDIO_HEIGHT,
							 global_x, global_y, global_xwidth, global_yheight,
							 0, 0, ghInst, 0);
	}
...
void OnPaint(HWND hwnd)
{
    ...
    {
        //FillRect(hdc, &rcClient, (HBRUSH)(COLOR_BTNFACE + 1)); //original
        FillRect(hdc, &rcClient, (HBRUSH)GetStockObject(BLACK_BRUSH)); //spi
    }
